...
class DeformConvFunction(Function):
    ...
    # FIXME: Couldn't export Python operator DeformConvFunction
    # https://github.com/zhangjinsong3/SOLOv2.tensorRT/issues/4
    @staticmethod
    def symbolic(g, input, offset, weight, stride, padding, dilation, groups, deformable_groups, im2col_step):
        return g.op(
            "MMCVDeformConv2d",
            input,
            offset,
...
class DeformConv1D(nn.Module):
    ...
    def __init__(self,
        ...
        self.groups = groups
        self.deformable_groups = deformable_groups
        self.im2col_step = im2col_step  # FIXME:
        # enable compatibility with nn.Conv2d
        self.transposed = False
        self.output_padding = _single(0)
    ...
    def forward(self, x, offset):
        ...
        if input_pad:
            ...
            offset = F.pad(offset, (0, pad_w, 0, pad_h), 'constant',
                           0).contiguous()
        out = deform_conv(x, offset, self.weight, self.stride, self.padding,
                          self.dilation, self.groups, self.deformable_groups, self.im2col_step)  # FIXME:
        if input_pad:
            out = out[:, :, :out.size(2) - pad_h, :out.size(3) -
                      pad_w].contiguous()
        return out
